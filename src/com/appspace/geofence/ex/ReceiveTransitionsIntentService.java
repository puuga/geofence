...
public class ReceiveTransitionsIntentService extends IntentService {
	...
	protected void onHandleIntent(Intent intent) {
		...
		if (LocationClient.hasError(intent)) {
			...
			if ((transition == Geofence.GEOFENCE_TRANSITION_ENTER)
				...
				// read place
				RoomlinkPlace[] roomlinkPlace = ApiManager
						.getRoomLinkPlace(getApplicationContext());
				Log.d("roomlink place", "count = " + roomlinkPlace.length);

				String locationName = roomlinkPlace[Integer.parseInt(ids) - 1]
						.getPlacename();

				sendNotification(transitionType, ids, locationName);

				// Log the transition type and a message
				Log.d(GeofenceUtils.APPTAG,
						getString(
								R.string.geofence_transition_notification_title,
								transitionType, ids, locationName));
				Log.d(GeofenceUtils.APPTAG,
	...
	private void sendNotification(String transitionType, String ids,
		...
		notificationIntent.putExtra("startFromNoti", true);
